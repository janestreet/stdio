...
(** [with_file ~f fname] executes [~f] on the open channel from [fname], and closes it
    afterwards. *)
...
(** [fold_lines ?fix_win_eol t ~init ~f] folds over the lines read from [t] using
    [input_line]. Lines are provided to [f] in the order they are found in the file. *)
val fold_lines
...
(** Completely reads an input channel and returns the results as a list of strings. Each
    line in one string. *)
...
(** [read_lines filename] reads the full contents of file and returns it as a list of
    lines, closing the file when it's done. It's the equivalent of
    [with_file fname ~f:input_lines] *)
val read_lines : ?fix_win_eol:bool -> string -> string list

(** [read_all filename] reads the full contents of file and returns it as a single string,
    closing the file when it's done. It's the equivalent of [with_file fname ~f:input_all] *)
